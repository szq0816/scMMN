...
class GCN(nn.Module):
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, **kwargs):
        ...
        if type(features) is not torch.Tensor:
            features, adj, labels = utils_1.to_tensor(features, adj, labels, device=self.device)
        ...
        if normalize:
            if utils_1.is_sparse_tensor(adj):
                adj_norm = utils_1.normalize_adj_tensor(adj, sparse=True)
            else:
                adj_norm = utils_1.normalize_adj_tensor(adj)
        else:
            adj_norm = adj
    ...
    def _train_with_val(self, labels, idx_train, idx_val, train_iters, verbose):
        ...
        for i in range(train_iters):
            ...
            acc_val = utils_1.accuracy(output[idx_val], labels[idx_val])
    ...
    def test(self, idx_test):
        ...
        acc_test = utils_1.accuracy(output[idx_test], self.labels[idx_test])
    ...
    def predict(self, features=None, adj=None):
        ...
        if features is None and adj is None:
            ...
            if type(adj) is not torch.Tensor:
                features, adj = utils_1.to_tensor(features, adj, device=self.device)

            self.features = features
            if utils_1.is_sparse_tensor(adj):
                self.adj_norm = utils_1.normalize_adj_tensor(adj, sparse=True)
            else:
                self.adj_norm = utils_1.normalize_adj_tensor(adj)
            return self.forward(self.features, self.adj_norm)
